...
Render's free web service has no built-in cron. Instead, `POST /api/run-sync` triggers the sync — protected by the `CRON_SECRET` env var (auto-generated by the Blueprint; find it in the service's Environment tab). Use a free external cron (cron-job.org) with **three jobs**:
...
Render's free tier spins the service down after 15 minutes of no traffic, and the first request after that takes 30–60s to wake. Two pings 5 minutes apart absorb the cold start: if the :45 ping aborts mid-wake (external crons have a 30s timeout and report an aborted connection as "output too large"), the :50 ping lands on a nearly-warm service. **Also disable "deactivate after failures" on these jobs** — their failure status is cosmetic; the wake still completes server-side.
...
The endpoint's hour guard (8–23 UTC) makes off-hours calls no-ops. Verify sync outcomes in the dashboard's History tab, not in cron-job.org — the History tab is the source of truth.
