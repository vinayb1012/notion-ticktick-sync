...
Render's free web service has no built-in cron. Instead, `POST /api/run-sync` triggers the sync — protected by the `CRON_SECRET` env var (auto-generated by the Blueprint; find it in the service's Environment tab). Use a free external cron (cron-job.org) with **two jobs**:

**Job 1 — warm-up (hourly at :50)**

- **URL**: `https://ticktick-notion-sync.onrender.com/api/health`
- **Method**: GET
- **Auth**: none (`/api/health` is public)

Render's free tier spins the service down after 15 minutes of no traffic, and the first request after that takes 30–60s. This ping wakes the service 10 minutes before the sync, so the sync itself always hits a warm service.

**Job 2 — sync (hourly at :00)**
...
The endpoint's hour guard (8–23 UTC) makes off-hours calls no-ops.

**Why not a single hourly job?** Without the warm-up, the hourly sync ping itself hits a cold start — and on cold starts the external cron can intermittently report failures (connection held 30–60s). The warm-up absorbs that.

**Instance-hours math:** awake ~15–20 min per hour ≈ 180–240 h/month — well within Render's 750 free hours. (A 24/7 keep-alive every 10 min would burn ~730 of 750 hours; this pattern gets the same sync reliability for a third of the cost. Note only the sync is protected — opening the dashboard after a long idle may still take 30–60s.)
